...
double Cubic::PSM_Energy(double epsilon){
...
double Cubic::PSM_EnergyChange(std::vector<double>& vecTrial, int idx, double epsilon){
  double deltaE = 0.;
  int nPart = coords.size();
  ...
  for(int i = 0; i < nPart; i++){
    ...
    if(pow(dr2_New, 0.5) < 1.)
      NewEng = epsilon;

    if(pow(dr2_Old, 0.5) < 1.)
      OldEng = epsilon;
...
void Cubic::MonteOptPSM(int N, double maxDir, int print, double epsilon){

  energy = PSM_Energy(epsilon);
  ...
  for(int n = 0; n < N; ++n){
	...
	trialIdx[j] = coords[i][j] + maxDir*(distribution(generator)-0.5);
      ...
      double deltaE = PSM_EnergyChange(trialIdx, i, epsilon);
